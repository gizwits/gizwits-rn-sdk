...
if (!isDeviceLoaded()) {
  ...
  if (!result && !isDeviceLoaded()) { throw new Error('JSI bindings were not installed for: RNGizwitsRnDevice Module'); }
...
global.GizDeviceListNotifications = debounce((data) => {
  setTimeout(() => {
    try {
      ...
      console.error('GizDeviceListNotifications callback error', error);
    }
  }, 1);
}, 600);
...
global.GizBleDeviceListNotifications = debounce((data) => {
  setTimeout(() => {
    try {
      ...
      console.error('GizBleDeviceListNotifications callback error', error);
    }
  }, 1);
}, 600);
...
global.GizDeviceStatusNotifications = (data) => {
  setTimeout(() => {
    try {
      ...
      console.error('GizDeviceStatusNotifications callback error', error);
    }
  }, 1);
};
