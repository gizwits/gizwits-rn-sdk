...
@see GizWifiErrorCode
**/
- (void)device:(GizWifiDevice * _Nonnull)device didReceiveAppToDevAttrStatus:(GizError * _Nonnull)result attrStatus:(NSDictionary * _Nullable)attrStatus withSN:(NSNumber * _Nullable)sn;
 ...
 */
- (void)device:(GizWifiDevice * _Nonnull)device didExitProductionTesting:(GizError * _Nonnull)result;
...
/**
 ...
 @see 对应回调接口：[GizWifiDeviceDelegate device:didSetSubscribe:isSubscribed:]
 */
- (void)setSubscribe:(BOOL)subscribed autoGetDeviceStatus:(BOOL)autoGetDeviceStatus;

/**
 获取硬件信息
 @see 对应的回调接口：[GizWifiDeviceDelegate device:didGetHardwareInfo:hardwareInfo:]
 */
...
/**
 ...
 @see 对应的回调接口：[GizWifiDeviceDelegate device:didReceiveData:attrStatus:withSN:]
 */
