...
RCT_EXPORT_METHOD(setDeviceOnboardingDeploy:(id)info result:(RCTResponseSenderBlock)result){
    ...
    if (softAPSSIDPrefixs != nil) {
        [[GizWifiSDK sharedInstance] setDeviceOnboardingDeploy:ssid key:key configMode:configMode softAPSSIDPrefixs:softAPSSIDPrefixs timeout:(int)timeout wifiGAgentType:gagentTypes bind:isbind];
    } else {
...
- (void)wifiSDK:(GizWifiSDK *)wifiSDK didSetDeviceOnboarding:(NSError *)result mac:(NSString *)mac did:(NSString *)did productKey:(NSString *)productKey{
    ...
    [self notiWithType:GizWifiRnResultTypeSetOnboardingNoti result:errDict ?: dataDict];
